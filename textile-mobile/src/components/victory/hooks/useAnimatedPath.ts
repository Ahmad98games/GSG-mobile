...
export const useAnimatedPath = (
  ...
  const fromPathSV = useSharedValue<SkPath | null>(currentPathProp.copy());
  // targetPathSV stores the SkPath object we are animating TO. Initialized with a copy.
  const targetPathSV = useSharedValue<SkPath | null>(currentPathProp.copy());

  // M11: Memory Cleanup
  React.useEffect(() => {
    return () => {
      fromPathSV.value = null;
      targetPathSV.value = null;
    };
  }, []);

  // This effect synchronizes prop changes to our shared values and triggers animations.
  React.useEffect(() => {
    if (!targetPathSV.value) return;
