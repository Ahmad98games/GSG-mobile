...
import { Packet, SocketOptions as EngineOptions } from "./SynapseTypes";
import { Encoder, Decoder } from "./SynapseParser";
...
const debug = (...args: any[]) => console.log('[SYNAPSE:MANAGER]', ...args);
const nextTick = (fn: Function, ...args: any[]) => setTimeout(fn, 0, ...args);
const installTimerFunctions = (obj: any, opts: any) => {
  obj.setTimeoutFn = setTimeout;
  obj.clearTimeoutFn = clearTimeout;
};

class Engine extends Emitter<any, any> {
  public transport = { writable: true };
  constructor(uri: string, opts: any) {
    super();
  }
  write(data: any, opts: any) {}
  close() {}
  _hasPingExpired() { return false; }
}
...
> extends Emitter<{}, {}, ManagerReservedEvents> {
   ...
   * @private
   */
  _readyState: "opening" | "open" | "closed" = "closed";
  /**
   * @private
   */
  ...
  public nsps: Record<string, Socket> = {};
  private subs: Array<ReturnType<typeof on>> = [];
  // @ts-ignore
  private backoff: Backoff = new Backoff();
  private setTimeoutFn: typeof setTimeout = setTimeout;
  private clearTimeoutFn: typeof clearTimeout = clearTimeout;
  private _reconnection: boolean = true;
  private _reconnectionAttempts: number = Infinity;
  private _reconnectionDelay: number = 1000;
  private _randomizationFactor: number = 0.5;
  private _reconnectionDelayMax: number = 5000;
  private _timeout: any = 20000;
   ...
   * @private
   */
  private ondecoded(packet: any): void {
