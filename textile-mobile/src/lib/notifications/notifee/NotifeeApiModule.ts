...
import type { NativeModuleConfig } from './NotifeeNativeModule';
import {
  isAndroid,
  isArray,
  isFunction,
...
if (isAndroid) {
  ...
  AppRegistry.registerHeadlessTask(kReactNativeNotifeeForegroundServiceHeadlessTask, () => {
    ...
    return ({ notification }: { notification: Notification }): Promise<void> => registeredForegroundServiceTask(notification);
  });
}
