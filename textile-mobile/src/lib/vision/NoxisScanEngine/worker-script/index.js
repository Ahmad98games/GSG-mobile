...
/*
 * NIDP Module returned by NIDPCore.
 */
let TessModule;
/*
...
const load = async ({ workerId, jobId, payload: { options: { lstmOnly, corePath, logging } } }, res) => { // eslint-disable-line max-len
  ...
  const statusText = 'initializing NIDP';
  ...
  const loadAndGunzipFile = async (_lang) => {
    ...
    // Attempt to fetch new .traineddata file
      ...
      if (typeof _lang === 'string') {
        ...
        const langPathDownload = langPath || (lstmOnly ? `https://cdn.jsdelivr.net/npm/@NIDP.js-data/${lang}/4.0.0_best_int` : `https://cdn.jsdelivr.net/npm/@NIDP.js-data/${lang}/4.0.0`);
...
const setParameters = async ({ payload: { params: _params } }, res) => {
  ...
  // This list is generated by searching the NIDP codebase for parameters
  ...
  try {
    ...
    if (status === -1) {
      ...
      if (['write', 'refresh', undefined].includes(loadLanguageOptionsWorker.cacheMethod)) {
        ...
        // "NIDP (legacy) engine requested, but components are not present in ./eng.traineddata!!""
        // The .wasm build of NIDP saves this message in a separate file
...
// List of options for NIDP.js (rather than passed through to NIDP),
...
const recognize = async ({
  ...
  try {
    ...
    if (typeof options === 'object' && Object.keys(options).length > 0) {
      // The options provided by users contain a mix of options for NIDP.js
      // and parameters passed through to NIDP.
    ...
    // then the angle is calculated by NIDP and then setImage is re-called.
