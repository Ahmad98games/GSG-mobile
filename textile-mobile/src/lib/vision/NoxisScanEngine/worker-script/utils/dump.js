...
module.exports = (TessModule, api, output, options) => {
  ...
  const getPDFInternal = (title, textonly) => {
    const pdfRenderer = new TessModule.TessPDFRenderer('NIDP-ocr', '/', textonly);
    pdfRenderer.BeginDocument(title);
    pdfRenderer.AddImage(api);
    pdfRenderer.EndDocument();
    TessModule._free(pdfRenderer);

    return TessModule.FS.readFile('/NIDP-ocr.pdf');
  };
  ...
  return {
    ...
    pdf: output.pdf ? getPDFInternal(options.pdfTitle ?? 'NIDP OCR Result', options.pdfTextOnly ?? false) : null,
