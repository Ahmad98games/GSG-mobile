      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.offlineContainer}>
          ...
          <Text style={styles.offlineDesc}>
            Make sure Noxis Hub is running on your PC and you are on the same WiFi network.
          </Text>
          <TouchableOpacity
            style={styles.pairBtn}
            onPress={() => router.push('/(auth)/pair')}
          >
            <Text style={styles.pairBtnText}>Scan QR to Pair</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
          ...
          renderItem={({ item }) => (
            <TouchableOpacity style={styles.historyItem} onPress={() => {
              ...
              <View style={styles.historyInfo}>
                ...
                <Text style={styles.historyMeta}>{item.sku_code} • {timeAgo(item.timestamp)}</Text>
              </View>
      ...
      <Animated.View style={[styles.bottomSheet, sheetStyle]}>
        ...
        <View style={styles.sheetContent}>
          {result?.notFound ? (
            ...
            const qty = Number(result.qty_on_hand || result.qty || 0);
            const reorderLevel = Number(result.reorder_level) || 100;
            const unit = result.unit || 'meters';
              ...
              <>
                ...
                <View style={styles.detailCard}>
                  <View style={styles.cardHeaderRow}>
                    <Text style={styles.cardSkuTitle}>{result.sku_code}</Text>
                    {result.isOffline && (
                  ...
                  <Text style={styles.cardLine}>📦 On Hand: <Text style={styles.boldText}>{qty} {unit}</Text></Text>
                  <Text style={styles.cardLine}>💰 Sale Price: <Text style={styles.boldText}>Rs. {Number(result.sale_price || 0).toLocaleString()}/{unit}</Text></Text>
                  <Text style={styles.cardLine}>⚠️ Reorder Level: <Text style={styles.boldText}>{reorderLevel} {unit}</Text></Text>
                  <View style={styles.statusLine}>
                    <Text style={styles.cardLine}>Status: </Text>
                    <View style={[styles.statusBadge, { backgroundColor: statusColor + '20', borderColor: statusColor }]}>
                      <Text style={[styles.statusBadgeText, { color: statusColor }]}>{statusIcon} {statusText}</Text>
                    </View>
                  </View>
                </View>

                {/* LARGE ACTION BUTTONS (56dp) */}
                <View style={styles.largeActionsContainer}>
                  <TouchableOpacity 
                    ...
                    onPress={() => {
                      closeBottomSheet();
                      router.push({ pathname: '/(app)/invoices/new', params: { barcode: result.barcode || result.sku_id } });
                    }}
                  >
                    <ShoppingCart size={18} color={THEME.colors.blue} />
                    <Text style={[styles.largeActionText, { color: THEME.colors.blue }]}>Create Invoice</Text>
                  </TouchableOpacity>

                  <TouchableOpacity 
                    style={[styles.largeActionBtn, { borderColor: THEME.colors.gold, backgroundColor: 'rgba(197, 160, 89, 0.1)' }]} 
                    onPress={() => {
                      Alert.alert(
                        'Adjust Stock',
                        `Quick stock adjustment for ${result.sku_code}.\nChoose transaction type:`,
                        [
                          { text: 'Cancel', style: 'cancel' },
                          { text: 'Add 50 units', onPress: () => Alert.alert('Success', 'Inventory increased.') },
                          { text: 'Deduct 50 units', onPress: () => Alert.alert('Success', 'Inventory decreased.') }
                        ]
                      );
                    }}
                  >
                    <Settings2 size={18} color={THEME.colors.gold} />
                    <Text style={[styles.largeActionText, { color: THEME.colors.gold }]}>Adjust Stock</Text>
                  </TouchableOpacity>
                  ...
                  <TouchableOpacity 
                    ...
                    onPress={() => {
                      closeBottomSheet();
                      router.push({ pathname: '/(app)/shared/scan-history', params: { code: result.sku_code } });
                    }}
                  >
